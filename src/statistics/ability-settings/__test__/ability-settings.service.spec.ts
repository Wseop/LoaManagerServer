...
describe('AbilitySettingsService', () => {
  ...
  beforeEach(async () => {
    ...
    abilitySettingModel = module.get<Model<AbilitySetting>>(
      getModelToken(AbilitySetting.name),
    );
  });

  describe('findAbilitySettings', () => {
    it('should return array of AbilitySetting', async () => {
      const result = await abilitySettingsService.findAbilitySettings(
        'className',
      );
  ...
  describe('upsertAbilitySetting', () => {
    it('should return an AbilitySetting', async () => {
      ...
      expect(
        jest.spyOn(abilitySettingModel, 'findOneAndUpdate'),
      ).toBeCalledTimes(1);
    });
  });
  ...
  describe('deleteAbilitySetting', () => {
    it('should return true', async () => {
      ...
      expect(jest.spyOn(abilitySettingModel, 'deleteOne')).toBeCalledTimes(1);
    });
  });
